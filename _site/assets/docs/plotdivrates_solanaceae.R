...
# Read output
output.sse<-read.table("BiSSE_solanaceae.log", header=TRUE)
# see what's inside
head(output.sse)
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual( values = c(cols[7],cols[1]))+xlim(-0.1,0.8)
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual( values = c(cols[7],cols[1]))+xlim(0,1.2)
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual( values = c(cols[2],cols[4]))+xlim(0,0.1)
multiplot(p1,p3,p5,p2,p4, cols=2)

############################################################################################
############################################################################################
### Plots for diversification rates Hisse 
output.sse<-read.table("HiSSE_solanaceae.log", header=TRUE)
head(output.sse)
...
trait.rates<-data.frame(dens=c(output.sse$rate_12,output.sse$rate_21) ,Type=rep(c("q_01","q_10"),each=length(output.sse$rate_21)))
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual( values = c(cols[7],cols[9],cols[1],cols[6]))+xlim(-1.5,2)
...
output.sse<-read.table("CID_solanaceae.log", header=TRUE)
head(output.sse)
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual( values =  c("yellow","gray"))+xlim(-1.5,2)
...
#comparing net diversification
multiplot(p3,p3.2,p3.3)
